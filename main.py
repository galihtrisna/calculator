...
root = tk.Tk()
root.geometry("375x800") #ukuran window
...
total_label= tk.Label(frame, text=total_expression, anchor=tk.E, bg="#22252D", fg="#FFFFFF", padx=24, font=("Poppins", 16), wraplength=375, justify="right")
total_label.pack(expand=True, fill='both')

label = tk.Label(frame, text=current_expression, anchor=tk.E, bg="#22252D", fg="#FFFFFF", padx=24, font=("Poppins", 40, "bold"), wraplength=375, justify="right")
...
for digit, grid_value in digits.items():
    button = tk.Button(btn_frame, text=str(digit), bg="#292D36", fg="#ffffff", font=("Poppins", 24,"bold"),borderwidth=0, command=lambda x=digit: add_digit(x))
    button.grid(row=grid_value[0], column=grid_value[1], sticky=tk.NSEW)


i = 0
for operator, symbol in operations.items():
    button = tk.Button(btn_frame, text=symbol, bg="#292D36", fg="#EB6666", font=("Poppins", 20),
                        borderwidth=0, command=lambda x=operator: add_operator(x))
    button.grid(row=i, column=4, sticky=tk.NSEW)
    i += 1


button_clear = tk.Button(btn_frame, text="C", bg="#292D36", fg="#26F4CE", font=("Poppins", 20),
                           borderwidth=0, command=clear)
button_clear.grid(row=0, column=1, sticky=tk.NSEW)
button_equal = tk.Button(btn_frame, text="=", bg="#ffffff", fg="#292D36", font=("Poppins", 20),
                           borderwidth=0, command=evaluate)
button_equal.grid(row=4, column=3, columnspan=2, sticky=tk.NSEW)
button_square = tk.Button(btn_frame, text="x\u00b2", bg="#292D36", fg="#26F4CE", font=("Poppins", 20),
                           borderwidth=0, command=square)
button_square.grid(row=0, column=2, sticky=tk.NSEW)
button_delete = tk.Button(btn_frame, text="\u232b", bg="#292D36", fg="#26F4CE", font=("Poppins", 20),borderwidth=0, command=delete)
